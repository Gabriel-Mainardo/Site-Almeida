...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div className="container mx-auto flex justify-between items-center h-14 md:h-16 relative">
          ...
          >
            <div className="relative flex items-center justify-center h-16 md:h-24 w-16 md:w-24 overflow-visible -mt-1 md:-mt-3">
              ...
              <img 
                ...
                className={`h-12 sm:h-16 md:h-20 ${logoLoaded ? 'animate-logo-fade-in' : 'opacity-0'}`}
                style={{ 
        ...
        <nav className="hidden md:flex space-x-1 lg:space-x-4 items-center mr-3">
          {sections.map((section) => (
            <a
              ...
              className={`nav-link text-sm lg:text-base ${activeSection === section.id ? 'active' : ''}`}
            >
              {section.label}
            </a>
          ))}
          <a 
            href="#contact" 
            className="btn-secondary py-1.5 px-3 ml-2 text-sm lg:py-2 lg:px-4"
        ...
        <div className="md:hidden flex items-center justify-center h-full">
          <button 
            className="text-gold z-20 p-2 rounded-full bg-plum-dark/70 backdrop-blur-sm hover:bg-plum-dark/80 transition-colors border border-gold/20 touch-target flex items-center justify-center"
          ...
          <div className="flex flex-col space-y-6 items-center">
            {sections.map((section) => (
              <a
                ...
                } text-lg relative touch-target py-1`}
              >
                {section.label}
                {activeSection === section.id && (
            ...
            <a 
              href="#contact" 
              className="btn-secondary text-center py-3 px-8 mt-4 text-base w-64"
              onClick={handleNavClick('contact')}
            >
              Solicite Orçamento
            </a>
